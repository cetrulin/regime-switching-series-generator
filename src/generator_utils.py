...
def prepare_raw_series(mode: str, ts: pd.DataFrame()):
    ...
    if mode == 'returns':  # log returns
        ts = pd.DataFrame(np.log(ts / ts.shift(1))).reset_index(drop=True)  # .dropna().reset_index(drop=True)
    else:  # standardization the dataset
        ts = pd.DataFrame(min_max_scaler.transform(ts)).reset_index(drop=True)  # .dropna().reset_index(drop=True)
